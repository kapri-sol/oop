...
class StarbucksCoffeMachine {
  private capsule: CoffeeType = null;

  inputCapsule(capsuleType: CoffeeType) {
...
class Barista {
  ...
  makeCoffee(starbucksCoffeeMachine: StarbucksCoffeMachine): Coffee {
    starbucksCoffeeMachine.inputCapsule(this.order);
    const coffee = starbucksCoffeeMachine.turnOn();
    starbucksCoffeeMachine.clearCapsule();
